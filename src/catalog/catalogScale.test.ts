...
async function catalogOf(count: number, store: CatalogStore = new MemoryCatalogStore()) {
...
describe('catalog scale', () => {
  ...
  it('keeps search proportional to the catalog, not to the matches', async () => {
    ...
    const found = await catalog.queryCatalogTimeline(pubkey, { search: 'video' });
  ...
  it('answers ordinary filters from the projection table alone', async () => {
    const counting = new CountingStore(new MemoryCatalogStore());
    const catalog = await catalogOf(500, counting);
    await catalog.queryCatalogTimeline(pubkey);

    counting.scans.length = 0;
    await catalog.queryCatalogTimeline(pubkey, { types: ['video'], search: 'video', availability: ['unknown'] });
    expect(counting.scans).toEqual(['timeline_projection']);

    counting.scans.length = 0;
    await catalog.queryCatalogTimeline(pubkey, { serverId: 'https://one.example' });
    ...
    await catalog.queryCatalogAssetIds({ serverId: 'https://one.example' });
    expect(counting.scans).toEqual(['asset_blob']);
    counting.scans.length = 0;
    // `hashFor` pads with zeros, so only the tail of the hash is selective.
    const byHash = await catalog.queryCatalogAssetIds({ hashTerms: [hashFor(3)] });
    expect(counting.scans).toEqual(['asset_blob']);
  ...
  it('hides projections left behind by an earlier run', async () => {
    ...
    const catalog = await catalogOf(10, store);
    await catalog.queryCatalogTimeline(pubkey);
    const [stale, ...rest] = await catalog.queryCatalogTimeline(pubkey);
    expect(rest.length).toBe(9);

    await store.put('timeline_projection', { ...stale, projectedAt: stale.projectedAt - 1 });
    expect((await catalog.queryCatalogTimeline(pubkey)).map(item => item.assetId)).not.toContain(stale.assetId);
  });
  ...
  it("reads one item's contents without scanning any table, at any catalog size", async () => {
    const readsFor = async (count: number) => {
      const counting = new CountingStore(new MemoryCatalogStore());
      const catalog = await catalogOf(count, counting);
      await catalog.queryCatalogTimeline(pubkey);
      const [first] = await catalog.queryCatalogTimeline(pubkey);
      counting.scans.length = 0;
      counting.reads.length = 0;
      const contents = await catalog.getCatalogAssetContents(first.assetId, 4);
  ...
  it('loads a whole asset detail without scanning any table', async () => {
    ...
    const [first] = await catalog.queryCatalogTimeline(pubkey);
    counting.scans.length = 0;
    const detail = await catalog.getCatalogTimelineAsset(pubkey, first.assetId);
