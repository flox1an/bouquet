...
const Login: React.FC = () => {
  ...
  return (
    <div className="flex flex-col justify-center items-center h-[80vh] gap-4">
      ...
      <Card className="w-full max-w-96 mt-8">
          ...
          <div className="space-y-4">
            {activeTab === 'extension' && (
              <>
                ...
                <Button className="w-full" onClick={handleExtensionLogin} disabled={loading}>
            ...
            {activeTab === 'qr' && <QRCodeLogin onLogin={() => setAutoLogin(true)} onError={err => setError(err)} />}

            {activeTab === 'bunker' && (
              <>
                <p className="text-sm text-muted-foreground">Login using a NIP-46 bunker connection string.</p>
                <Input
                  type="text"
                  placeholder="bunker://..."
                  value={bunkerUri}
            ...
            {activeTab === 'nsec' && (
              <>
                <p className="text-sm text-muted-foreground">Login with your private key. Your key is never stored.</p>
                <Input
                  type="password"
                ...
                <Button className="w-full" onClick={handleNsecLogin} disabled={loading || !nsecKey.trim()}>
                  {loading && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
                  Login with Nsec
                </Button>
              </>
